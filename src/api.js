...
export const getImagePlayerIdExt = async (id) => {
  try {
    ...
    const { data } = await axios.get(
      `https://transfermarkt-api.vercel.app/players/${id}/profile`
    );
...
export const getPlayerById = async (id) => {
  const { data } = await axios.get(`http://dev39-kub:8080/players/${id}`);
  const imageUrl = await getImagePlayerIdExt(data.external_id);
  return { ...data, imageUrl };
};

export const getClubById = async (id) => {
  const { data } = await axios.get(`http://dev39-kub:8080/clubs/${id}`);
  return data;
};
...
export const getAllLeagues = async () => {
  const { data } = await axios.get("http://dev39-kub:8080/leagues");

  return data;
};

export const getAllClubs = async () => {
  const { data } = await axios.get("http://dev39-kub:8080/clubs");
